...
class HrResignation(models.Model):
    """Model for HR Resignations. This model is used to track employee resignations."""
    ...
    expected_revealing_date = fields.Date(string="Last Day of Employee", required=True,
                                          help='Employee requested date on which employee is '
                                               'revealing from the company.')
    ...
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirm'), ('approved', 'Approved'),
                              ('cancel', 'Rejected')], string='Status', default='draft', track_visibility="always")
    resignation_type = fields.Selection(selection=RESIGNATION_TYPE,
                                        help="Select the type of resignation: normal resignation or "
                                             "fired by the company")
    change_employee = fields.Boolean(string="Change Employee", compute="_compute_change_employee",
    ...
    def _check_employee_id(self):
        """Check whether the current user has permission to create a resignation request."""
        for resignation in self:
            if not self.env.user.has_group('hr.group_hr_user'):
                if resignation.employee_id.user_id.id and resignation.employee_id.user_id.id != self.env.uid:
    ...
    def _check_joined_date(self):
        """Check for an active resignation request for the same employee.

        This verifies whether another resignation exists in confirmed or approved
        state, based on the `joined_date` of the current resignation.
        """
        for resignation in self:
            resignation_request = self.env['hr.resignation'].search(
                [('employee_id', '=', resignation.employee_id.id), ('state', 'in', ['confirm', 'approved'])])
    ...
    def _onchange_employee_id(self):
        ...
        if self.employee_id:
            resignation_request = self.env['hr.resignation'].search(
                [('employee_id', '=', self.employee_id.id), ('state', 'in', ['confirm', 'approved'])])
            if resignation_request:
    ...
    def action_confirm_resignation(self):
        """Triggered by the 'Confirm' button to confirm the resignation request."""
        for resignation in self:
            if resignation.joined_date:
                if resignation.joined_date >= resignation.expected_revealing_date:
    ...
    def action_cancel_resignation(self):
        """Triggered by the 'Cancel' button to cancel the resignation request."""
        for resignation in self:
            resignation.state = 'cancel'

    def action_reject_resignation(self):
        """Triggered by the 'Reject' button to reject the resignation request."""
        for resignation in self:
            resignation.state = 'cancel'

    def action_reset_to_draft(self):
        """Triggered by the 'Set to Draft' button to reset the resignation request to the 'draft' state."""
        for resignation in self:
    ...
    def action_approve_resignation(self):
        """Triggered by the 'Approve' button to approve the resignation."""
        for resignation in self:
            if resignation.expected_revealing_date and resignation.resign_confirm_date:
                ...
                for contract in employee_contract:
                    if contract.state == 'open':
                        ...
                        resignation.approved_revealing_date = (resignation.resign_confirm_date + timedelta(
                            days=contract.notice_days))
                    else:
                        resignation.approved_revealing_date = resignation.expected_revealing_date
                    # Cancelling contract
                    contract.state = 'cancel' if contract.state == "open" else contract.state
                # Changing state of the employee if resigning today
                if resignation.expected_revealing_date <= fields.Date.today() and resignation.employee_id.active:
                    ...
                    if resignation.resignation_type == 'resigned':
                        ...
                        departure_reason_id = self.env['hr.departure.reason'].search([('name', '=', 'Resigned')])
                    else:
                        resignation.employee_id.fired = True
                        departure_reason_id = self.env['hr.departure.reason'].search([('name', '=', 'Fired')])
